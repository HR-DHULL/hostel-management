...
export const metadata: Metadata = { title: 'Hostel Fees' }
...
export default async function HostelFeesPage({ searchParams }: PageProps) {
  const now   = new Date()
  ...
  const waTemplate = (settings as any)?.wa_template_fee_reminder
    ?? 'Hi {name}, your hostel fee of {amount} is due on {date} for {month}. Please pay at the earliest. - Management'

  const [fees, summary] = await Promise.all([
    getOrGenerateFees('hostel', month, year),
    getMonthSummary('hostel', month, year),
  ])
