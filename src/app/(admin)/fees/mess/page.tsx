...
export const metadata: Metadata = { title: 'Mess Fees' }
...
export default async function MessFeesPage({ searchParams }: PageProps) {
  const now   = new Date()
  ...
  const waTemplate = (settings as any)?.wa_template_fee_reminder
    ?? 'Hi {name}, your mess fee of {amount} is due on {date} for {month}. Please pay at the earliest. - Management'

  const [fees, summary] = await Promise.all([
    getOrGenerateFees('mess', month, year),
    getMonthSummary('mess', month, year),
  ])
  ...
  return (
    <>
      <Topbar
        ...
        actions={
          <div className="flex items-center gap-2">
            <ResetMonthButton module="mess" month={month} year={year} />
      ...
      <div className="p-6 space-y-5">
        ...
        <FeeTable fees={fees} module="mess" month={month} year={year} waTemplate={waTemplate} />
      </div>
    </>
  )
}
